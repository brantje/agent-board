...
func (c *fakeManagerClient) Capabilities() protocol.Capabilities {
	return protocol.Capabilities{MaxActiveSessions: 1}
}
func (c *fakeManagerClient) Health() protocol.Health { return c.health }
func (c *fakeManagerClient) Start(context.Context, string, Request) (ProcessSession, error) {
	return &fakeProcessSession{id: "started"}, nil
}
func (c *fakeManagerClient) Attach(id string) (ProcessSession, error) {
	c.attachCalls.Add(1)
	return &fakeProcessSession{id: id}, nil
}
...
func TestNewManagerConstructsDefaultDialer(t *testing.T) {
	if _, err := NewManager(nil, nil); err == nil {
		t.Fatal("nil resolver accepted")
	}
	manager, err := NewManager(&fakeEndpointResolver{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	defer manager.Close()
