...
func (s *runEvidenceTestStore) GetRawOutputChunk(_ context.Context, projectID, runID, chunkID string) (store.RawOutputChunk, error) {
	for _, chunk := range s.chunks {
		if chunk.ProjectID == projectID && chunk.RunID == runID && chunk.ID == chunkID {
			return chunk, nil
		}
	}
...
func (s *runEvidenceTestStore) GetArtifact(_ context.Context, projectID, runID, artifactID string) (store.Artifact, error) {
	for _, artifact := range s.artifacts {
		if artifact.ProjectID == projectID && artifact.RunID == runID && artifact.ID == artifactID {
			return artifact, nil
		}
	}
...
func TestRunEvidenceInspectReconstructsCompleteRun(t *testing.T) {
	...
	storeFake := &runEvidenceTestStore{
		...
		sessions: []store.ExecutionSession{
			{ID: "session", ProjectID: projectID, RunID: runID, RunnerID: runnerID, CommandArgv: json.RawMessage(`["go","test","./..."]`)},
			{ID: "other-session", ProjectID: projectID, RunID: "other-run", RunnerID: runnerID},
		},
	...
	for i := int64(1); i <= 501; i++ {
		sequence := i
		storeFake.events = append(storeFake.events, store.Event{ID: "event", ProjectID: projectID, RunID: runEvidenceStringPointer(runID), Sequence: &sequence, Type: "tool.completed"})
	}
	...
	if len(got.Sessions) != 1 || got.Sessions[0].ID != "session" || got.Sessions[0].RunnerID != runnerID {
...
func TestRunEvidenceAllowsRunBeforeProvenanceExists(t *testing.T) {
	storeFake := &runEvidenceTestStore{run: store.Run{ID: "run", ProjectID: "project"}}
	blobs, err := evidence.NewFileBlobStore(t.TempDir(), 1024)
	if err != nil {
		t.Fatal(err)
	}
