...
func (e *Engine) Execute(ctx context.Context, request engine.Request) (engine.Result, error) {
	...
	steps := []engine.ProcessRequest{
		...
		{Kind: "test", Name: "scripted-fixture", CWD: "/workspace", Command: []string{"sh", "-lc", `test -f staged.txt && test -f new-scripted.txt && test -f ignored-scripted.txt && test ! -e delete.txt && (test -f renamed.txt || test -f rename.txt) && printf 'scripted test passed\n'`}},
	}
