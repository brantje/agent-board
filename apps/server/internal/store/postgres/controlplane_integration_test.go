...
func TestControlPlanePersistenceAndProjectIsolation(t *testing.T) {
	...
	if _, err = s.UpdateProject(ctx, p1); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = s.GetProvider(ctx, nil, provider.ID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = s.UpdateProvider(ctx, nil, provider); err != nil {
		t.Fatal(err)
	}
	if _, err = s.CreateProvider(ctx, store.Provider{Name: "Provider", Kind: "test", Enabled: true}); !errors.Is(err, store.ErrConflict) {
	...
	if _, err = s.GetModelProfile(ctx, scope1, model.ID); err != nil {
		t.Fatal(err)
	}
	if _, err = s.GetModelProfile(ctx, scope2, model.ID); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("cross-project model err=%v", err)
	}
	...
	if _, err = s.UpdateModelProfile(ctx, scope1, model); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = s.GetAgentInScope(ctx, scope1, agent.ID); err != nil {
		t.Fatal(err)
	}
	if _, err = s.GetAgentInScope(ctx, scope2, agent.ID); !errors.Is(err, store.ErrNotFound) {
		t.Fatalf("cross-project agent err=%v", err)
	}
	...
	if workspace.BootstrapStatus != "PENDING" || !strings.HasPrefix(workspace.Path, pendingWorkspacePrefix) {
	...
	if err = pool.QueryRow(ctx, `SELECT count(*) FROM scheduler_jobs WHERE run_id=$1 AND kind='START' AND state='QUEUED'`, run.ID).Scan(&jobCount); err != nil {
		t.Fatal(err)
	}
