...
func TestCannotDisableLastActiveDeploymentAdmin(t *testing.T) {
	s := New(testPool(t))
...
func TestConcurrentDisableCannotRemoveLastTwoAdmins(t *testing.T) {
	s := New(testPool(t))
	ctx := context.Background()
...
func TestCreatePendingUserAndSetupTokenRollBackTogether(t *testing.T) {
	pool := testPool(t)
	s := New(pool)
	ctx := context.Background()

	if _, err := pool.Exec(ctx, `
		CREATE OR REPLACE FUNCTION fail_setup_token_insert() RETURNS trigger AS $$
		BEGIN
			RAISE EXCEPTION 'forced setup token insert failure';
		END;
		$$ LANGUAGE plpgsql;
		CREATE TRIGGER fail_setup_token_insert
		BEFORE INSERT ON password_tokens
		FOR EACH ROW EXECUTE FUNCTION fail_setup_token_insert();
	`); err != nil {
		t.Fatal(err)
	}
	defer func() {
		_, _ = pool.Exec(context.Background(), `DROP TRIGGER IF EXISTS fail_setup_token_insert ON password_tokens`)
		_, _ = pool.Exec(context.Background(), `DROP FUNCTION IF EXISTS fail_setup_token_insert()`)
	}()
...
func TestUpdateUserIdentityMissingRowUsesStoreNotFound(t *testing.T) {
	s := New(testPool(t))
	ctx := context.Background()
