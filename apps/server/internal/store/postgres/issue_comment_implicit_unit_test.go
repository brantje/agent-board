...
func TestSameIssueCommentImplicitRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "explicit mentions accept no trigger", enabled: true, hasExplicitMentions: true, want: true},
		{name: "explicit mentions reject implicit trigger", existing: queued, enabled: true, hasExplicitMentions: true},
		{name: "enabled accepts unresolved route", enabled: true, want: true},
		{name: "suppressed replay matches suppressed outcome", existing: suppressed, enabled: true, suppress: true, want: true},
		{name: "suppressed replay rejects queued outcome", existing: queued, enabled: true, suppress: true},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if got := sameIssueCommentImplicitRequest(test.existing, test.enabled, test.suppress, test.hasExplicitMentions); got != test.want {
