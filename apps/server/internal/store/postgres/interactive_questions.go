...
func (s *Store) OpenInteractiveQuestions(ctx context.Context, inputs []store.OpenInteractiveQuestionCommand) (store.OpenInteractiveQuestionsResult, error) {
	...
	for index := range results {
		event, ensureErr := ensureInteractiveQuestionCreatedEvent(ctx, tx, run, results[index].Question)
		if ensureErr != nil {
	...
	if enteredWaiting {
		...
		waitingEvent, eventErr := appendInteractiveQuestionEvent(ctx, tx, run, "run.waiting_for_input", map[string]any{
...
func ensureInteractiveQuestionCreatedEvent(ctx context.Context, tx pgx.Tx, run store.Run, question store.Question) (store.Event, error) {
	existing, err := scanEvent(tx.QueryRow(ctx, `
		SELECT id::text, schema_version, type, occurred_at, project_id::text, issue_id::text,
		       run_id::text, agent_id::text, workspace_id::text,
	...
	return appendInteractiveQuestionEvent(ctx, tx, run, "question.created", map[string]any{
...
func appendInteractiveQuestionEvent(ctx context.Context, tx pgx.Tx, run store.Run, eventType string, payload any) (store.Event, error) {
