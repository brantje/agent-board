...
func TestSquadStoreSerializesWithAgentOwnershipChanges(t *testing.T) {
	t.Run("ownership change first", func(t *testing.T) {
		...
		if _, err := ownerTx.Exec(ctx, `UPDATE agents SET project_id = $1, model_profile_id = $2 WHERE id = $3`, projectB.ID, agentsB[0].ModelProfileID, agentsA[1].ID); err != nil {
		...
		_, err = s.CreateSquad(attemptCtx, store.Squad{
			ProjectID: projectA.ID, Name: "Blocked create", LeaderAgentID: agentsA[0].ID,
		...
		if err := ownerTx.Commit(ctx); err != nil { t.Fatal(err) }
		_, err = s.CreateSquad(ctx, store.Squad{
			ProjectID: projectA.ID, Name: "Rejected create", LeaderAgentID: agentsA[0].ID,
			Members: []store.SquadMember{{Type: store.SquadMemberTypeAgent, ID: agentsA[1].ID}},
		})
	...
	t.Run("Squad reference first", func(t *testing.T) {
		...
		input := store.Squad{
			ProjectID: projectA.ID, Name: "Held reference", LeaderAgentID: agentsA[0].ID,
			Members: []store.SquadMember{{Type: store.SquadMemberTypeAgent, ID: agentsA[1].ID}},
		}
		...
		var squadID string
		if err := refTx.QueryRow(ctx, `INSERT INTO squads (project_id, name, leader_agent_id) VALUES ($1, $2, $3) RETURNING id::text`, input.ProjectID, input.Name, input.LeaderAgentID).Scan(&squadID); err != nil {
			t.Fatalf("stage Squad: %v", err)
		}
		if _, err := refTx.Exec(ctx, `INSERT INTO squad_members (squad_id, agent_id) VALUES ($1, $2)`, squadID, agentsA[1].ID); err != nil {
		...
		if err != nil { t.Fatal(err) }
		defer ownerTx.Rollback(ctx) //nolint:errcheck
		if _, err := ownerTx.Exec(ctx, `SET LOCAL lock_timeout = '100ms'`); err != nil { t.Fatal(err) }
		_, err = ownerTx.Exec(ctx, `UPDATE agents SET project_id = $1, model_profile_id = $2 WHERE id = $3`, projectB.ID, agentsB[0].ModelProfileID, agentsA[1].ID)
		assertSquadPostgresCode(t, err, "55P03")
		if err := ownerTx.Rollback(ctx); err != nil { t.Fatal(err) }

		if err := refTx.Commit(ctx); err != nil { t.Fatal(err) }
		_, err = pool.Exec(ctx, `UPDATE agents SET project_id = $1, model_profile_id = $2 WHERE id = $3`, projectB.ID, agentsB[0].ModelProfileID, agentsA[1].ID)
...
func TestSquadDatabaseSerializesDirectReferencesWithAgentOwnershipChanges(t *testing.T) {
	t.Run("member waits for Project ownership change", func(t *testing.T) {
		...
		squad, err := s.CreateSquad(ctx, store.Squad{ProjectID: projectA.ID, Name: "Direct member lock", LeaderAgentID: agentsA[0].ID})
		if err != nil { t.Fatal(err) }
		...
		if _, err := memberTx.Exec(ctx, `SET LOCAL lock_timeout = '100ms'`); err != nil { t.Fatal(err) }
		_, err = memberTx.Exec(ctx, `INSERT INTO squad_members (squad_id, agent_id) VALUES ($1, $2)`, squad.ID, agentsA[1].ID)
		assertSquadPostgresCode(t, err, "55P03")
		if err := memberTx.Rollback(ctx); err != nil { t.Fatal(err) }

		if err := ownerTx.Commit(ctx); err != nil { t.Fatal(err) }
		_, err = pool.Exec(ctx, `INSERT INTO squad_members (squad_id, agent_id) VALUES ($1, $2)`, squad.ID, agentsA[1].ID)
		assertSquadPostgresCode(t, err, "23514")
	})
	...
	t.Run("global leader blocks Project ownership change", func(t *testing.T) {
		...
		squad, err := s.CreateSquad(ctx, store.Squad{ProjectID: projectA.ID, Name: "Direct leader lock", LeaderAgentID: agentsA[0].ID})
		if err != nil { t.Fatal(err) }

		refTx, err := pool.Begin(ctx)
		if err != nil { t.Fatal(err) }
		defer refTx.Rollback(ctx) //nolint:errcheck
		if _, err := refTx.Exec(ctx, `UPDATE squads SET leader_agent_id = $1 WHERE id = $2`, globalAgent.ID, squad.ID); err != nil {
		...
		defer ownerTx.Rollback(ctx) //nolint:errcheck
		if _, err := ownerTx.Exec(ctx, `SET LOCAL lock_timeout = '100ms'`); err != nil { t.Fatal(err) }
		_, err = ownerTx.Exec(ctx, `UPDATE agents SET project_id = $1 WHERE id = $2`, projectB.ID, globalAgent.ID)
		assertSquadPostgresCode(t, err, "55P03")
		if err := ownerTx.Rollback(ctx); err != nil { t.Fatal(err) }

		if err := refTx.Commit(ctx); err != nil { t.Fatal(err) }
		_, err = pool.Exec(ctx, `UPDATE agents SET project_id = $1 WHERE id = $2`, projectB.ID, globalAgent.ID)
		assertSquadPostgresCode(t, err, "23514")
	})
}
...
func TestSquadStoreRechecksEnabledAgentAfterLockWait(t *testing.T) {
	...
	if agents[1].State != "ENABLED" { t.Fatalf("fixture Agent state = %q, want ENABLED", agents[1].State) }
	...
	go func() {
		_, err := s.CreateSquad(ctx, store.Squad{
			ProjectID: project.ID, Name: "Disabled race", LeaderAgentID: agents[0].ID,
	...
	if err := disableTx.Commit(ctx); err != nil { t.Fatal(err) }
	select {
	case err := <-result:
		if !errors.Is(err, store.ErrInvalidArgument) {
...
func assertSquadPostgresCode(t *testing.T, err error, code string) {
	t.Helper()
	if err == nil { t.Fatalf("expected PostgreSQL error %s", code) }
