...
func (s *Server) registerReviewTools(server *mcp.Server) {
	schemas := lockedReviewToolInputSchemas()
	listReviews := readOnlyTool("list_reviews", "List Reviews with optional public Issue key and Review status filters.")
	listReviews.InputSchema = schemas.listReviews
	mcp.AddTool(server, listReviews, objectListHandler(s.listReviews))
