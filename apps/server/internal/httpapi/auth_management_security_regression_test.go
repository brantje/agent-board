...
type authLifecycleHTTPStore struct {
	*authHTTPStore
}

func (s *authLifecycleHTTPStore) SetUserPassword(_ context.Context, id, hash string, force bool) (store.User, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	user, ok := s.users[id]
	if !ok {
...
func newAuthLifecycleHTTPHandler(t *testing.T, now *time.Time) http.Handler {
	t.Helper()
	authStore := &authLifecycleHTTPStore{authHTTPStore: newAuthHTTPStore()}
...
func TestPendingDirectPasswordActivatesAndRevokesSetupHTTP(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
	handler := newAuthLifecycleHTTPHandler(t, &now)
...
func TestOneTimePasswordTokenPlaintextOnlyAtResponseBoundary(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
	...
	if createdResponse.Code != http.StatusCreated || createdResponse.Header().Get("Cache-Control") != "no-store" {
		t.Fatalf("create pending status=%d cache=%q body=%s", createdResponse.Code, createdResponse.Header().Get("Cache-Control"), createdResponse.Body.String())
	}
	var created pendingUserResponse
	if err := json.Unmarshal(createdResponse.Body.Bytes(), &created); err != nil {
		t.Fatal(err)
...
func TestForcedPasswordChangeBlocksNormalAndAdminHTTPAccess(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
	handler, _, _ := newAuthHTTPHandler(t, &now)
	registerAuthHTTPUser(t, handler)
	admin := loginAuthHTTPUser(t, handler)
...
func TestInvalidAuthSettingsUsesApplicationValidation(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
	handler, _, _ := newAuthHTTPHandler(t, &now)
	registerAuthHTTPUser(t, handler)
	admin := loginAuthHTTPUser(t, handler)
