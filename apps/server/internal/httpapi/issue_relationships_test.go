...
func (s *issueRelationshipHTTPStore) GetIssue(_ context.Context, pid, id string) (store.Issue, error) {
	...
	switch id {
	case issueID:
		return store.Issue{ID: issueID, ProjectID: projectID, Key: issueKey, Number: 1, Title: "Source", Status: "TODO", Priority: 2}, nil
	case otherID:
		return store.Issue{ID: otherID, ProjectID: projectID, Key: otherIssueKey, Number: 2, Title: "Target", Status: "BACKLOG", Priority: 1}, nil
	default:
		return store.Issue{}, store.ErrNotFound
	}
}
...
func TestIssuePriorityAndRelationshipHTTPContract(t *testing.T) {
	...
	if relationship.SourceIssueID != issueKey || relationship.TargetIssueID != otherIssueKey || relationship.Type != "blocks" {
		t.Fatalf("relationship=%+v", relationship)
	}

	list := httptest.NewRecorder()
	router.ServeHTTP(list, httptest.NewRequest(http.MethodGet, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships", nil))
	...
	router.ServeHTTP(duplicate, httptest.NewRequest(http.MethodPost, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships", strings.NewReader(`{"targetIssueId":"`+otherIssueKey+`","type":"blocks"}`)))
	assertAPIError(t, duplicate, http.StatusConflict, "issue_relationship_exists")
	backend.duplicate = false

	self := httptest.NewRecorder()
	router.ServeHTTP(self, httptest.NewRequest(http.MethodPost, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships", strings.NewReader(`{"targetIssueId":"`+issueKey+`","type":"related_to"}`)))
	assertAPIError(t, self, http.StatusBadRequest, "issue_relationship_self_reference")

	invalidTargetID := httptest.NewRecorder()
	router.ServeHTTP(invalidTargetID, httptest.NewRequest(http.MethodPost, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships", strings.NewReader(`{"targetIssueId":"not-a-uuid","type":"blocks"}`)))
	assertAPIError(t, invalidTargetID, http.StatusBadRequest, "invalid_id")

	missingTarget := httptest.NewRecorder()
	router.ServeHTTP(missingTarget, httptest.NewRequest(http.MethodPost, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships", strings.NewReader(`{"targetIssueId":"`+missingIssueKey+`","type":"blocks"}`)))
	assertAPIError(t, missingTarget, http.StatusNotFound, "issue_not_found")

	invalidType := httptest.NewRecorder()
	router.ServeHTTP(invalidType, httptest.NewRequest(http.MethodPost, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships", strings.NewReader(`{"targetIssueId":"`+otherIssueKey+`","type":"parents"}`)))
	assertAPIError(t, invalidType, http.StatusBadRequest, "invalid_argument")

	invalidBody := httptest.NewRecorder()
	router.ServeHTTP(invalidBody, httptest.NewRequest(http.MethodPost, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships", strings.NewReader(`{"targetIssueId":`)))
	...
	router.ServeHTTP(missingSource, httptest.NewRequest(http.MethodGet, "/api/projects/"+projectID+"/issues/"+missingIssueKey+"/relationships", nil))
	assertAPIError(t, missingSource, http.StatusNotFound, "issue_not_found")

	remove := httptest.NewRecorder()
	router.ServeHTTP(remove, httptest.NewRequest(http.MethodDelete, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships/"+modelID, nil))
	if remove.Code != http.StatusNoContent {
		t.Fatalf("delete status=%d body=%s", remove.Code, remove.Body.String())
	}

	missingRelationship := httptest.NewRecorder()
	router.ServeHTTP(missingRelationship, httptest.NewRequest(http.MethodDelete, "/api/projects/"+projectID+"/issues/"+issueKey+"/relationships/"+modelID, nil))
