...
func TestDelegationInspectionRoutesAreReadOnly(t *testing.T) {
	...
	if len(listed) != 1 || listed[0].ID != delegationFixture().ID || listed[0].WorkspaceAccess != store.DelegationWorkspaceAccessWrite ||
		listed[0].ParentRunID == nil || *listed[0].ParentRunID != runID || listed[0].ParentAgentID == nil || *listed[0].ParentAgentID != agentID ||
		listed[0].SourceCommentID != nil || listed[0].ParentRunStatus == nil || *listed[0].ParentRunStatus != "PAUSED" ||
		listed[0].DelegatedRunStatus != "COMPLETED" || listed[0].Outcome == nil || *listed[0].Outcome != store.DelegationOutcomeSucceeded ||
		listed[0].ResultSummary == nil || *listed[0].ResultSummary != "bounded result" || listed[0].ResultEventID == nil ||
		listed[0].WorkspaceChangesAccepted == nil || !*listed[0].WorkspaceChangesAccepted || listed[0].WorkspaceRevision != "revision-1" ||
		listed[0].ContinuationJobID == nil || listed[0].CompletedAt == nil {
	...
	if child.ParentRunID == nil || *child.ParentRunID != runID || child.ParentAgentID == nil || *child.ParentAgentID != agentID ||
		child.SourceCommentID != nil || child.DelegatedRunID != delegatedRunID || child.WorkspaceAccess != store.DelegationWorkspaceAccessWrite ||
		child.ParentRunStatus == nil || *child.ParentRunStatus != "PAUSED" || child.DelegatedRunStatus != "COMPLETED" ||
		child.Outcome == nil || *child.Outcome != store.DelegationOutcomeSucceeded || child.WorkspaceRevision != "revision-1" {
