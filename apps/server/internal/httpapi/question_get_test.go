...
func TestQuestionRouteGetsSingleQuestionAndValidatesFilters(t *testing.T) {
	...
	router.ServeHTTP(get, httptest.NewRequest(http.MethodGet, "/api/projects/"+projectID+"/questions/"+otherID, nil))
	...
	router.ServeHTTP(invalidID, httptest.NewRequest(http.MethodGet, "/api/projects/"+projectID+"/questions/not-a-uuid", nil))
	...
	router.ServeHTTP(invalidIssue, httptest.NewRequest(http.MethodGet, "/api/projects/"+projectID+"/questions?issueId=bad", nil))
	...
	router.ServeHTTP(invalidRun, httptest.NewRequest(http.MethodGet, "/api/projects/"+projectID+"/questions?runId=bad", nil))
	...
	router.ServeHTTP(invalidStatus, httptest.NewRequest(http.MethodGet, "/api/projects/"+projectID+"/questions?status=unknown", nil))
