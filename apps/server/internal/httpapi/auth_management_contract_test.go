...
func TestSelfPasswordChangeRequiresCurrentPasswordUnlessForced(t *testing.T) {
...
func TestForcedPasswordChangeAcceptsNewPasswordOnly(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
...
func TestAuthSettingsRejectEveryOmittedRequiredField(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
	handler, _, _ := newAuthHTTPHandler(t, &now)
	registerAuthHTTPUser(t, handler)
	admin := loginAuthHTTPUser(t, handler)
...
func TestPrivilegedAuthGETResponsesAreNoStore(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
	handler, _, _ := newAuthHTTPHandler(t, &now)
	registerAuthHTTPUser(t, handler)
	admin := loginAuthHTTPUser(t, handler)
