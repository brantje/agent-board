...
async function openProjectPage(page, projectId, path) {
  const roleResponse = page.waitForResponse(response => response.url().includes(`/api/projects/${projectId}/access/effective-role`), { timeout: 15_000 }).catch(() => undefined)
...
try {
  ...
  const admin = await login('phase5-admin', passwords.admin)
  assert.equal(admin.user.deploymentRole, 'admin')

  const creator = await createActiveMember(admin.accessToken, 'phase5-creator', passwords.creator)
  const collaborator = await createActiveMember(admin.accessToken, 'phase5-collaborator', passwords.collaborator)
  ...
  const project = await call('POST', '/api/projects', {
    ...
    body: {
      name: 'Phase 5 Private',
      issuePrefix: 'E2E',
      sourceType: 'local',
      repositoryPath: '/repositories/.phase5-auth-e2e',
      defaultBranch: 'main',
...

  {
    ...
    await page.getByRole('heading', { name: /Phase 5 Private \/ Board/ }).waitFor()
  ...
  const group = await call('POST', '/api/groups', {
    ...
    body: { name: 'phase5-collaborators' },
    status: 201
  })
  ...
  const renamed = await call('PATCH', `/api/projects/${project.id}`, {
    token: collaborator.tokens.accessToken,
    body: { name: 'Phase 5 Private Updated' }
  })
  assert.equal(renamed.name, 'Phase 5 Private Updated')
  ...
  const extraSession = await login('phase5-collaborator', passwords.collaborator)
  ...
  const beforeDisable = await login('phase5-collaborator', passwords.collaborator)
  ...
  const afterEnable = await login('phase5-collaborator', passwords.collaborator)
  ...
  console.log('Phase 5 Playwright authorization flow passed')
} finally {
  await browser.close()
  await api.dispose()
}
