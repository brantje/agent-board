...
describe('RunDetail', () => {
  it('renders the two-region working-state layout, evidence disclosure, and live timeline', async () => {
    ...
    expect(sidebar.get('[data-run-agent]').text()).toContain('lab-host')
    expect(sidebar.get('[data-run-agent]').text()).not.toContain('Runtime')
    ...
    expect(sidebar.text().indexOf('Changed files')).toBeLessThan(sidebar.text().indexOf('Provenance'))
    expect(sidebar.text().indexOf('Provenance')).toBeLessThan(sidebar.text().indexOf('Artifacts'))
    expect(sidebar.text()).not.toContain('Runtime instances')
