    ...
    const fetch = vi.fn(async (path: string, options: RequestInit = {}) => {
      if (path.endsWith('/approve')) {
        return new Response(JSON.stringify({
          ...
          issue: { id: 'AB-1', projectId: 'project-a', number: 1, title: 'Fix', description: '', status: 'DONE', priority: 0, assignedTo: { type: 'AGENT', id: 'agent-1', name: 'Agent' }, createdAt: '', updatedAt: '', lastEvent: null }
        }))
      }
    ...
    const fetch = vi.fn(async (path: string, options: RequestInit = {}) => {
      if (path.endsWith('/request-changes')) {
        ...
        return new Response(JSON.stringify({
          ...
          issue: { id: 'AB-1', projectId: 'project-a', number: 1, title: 'Fix', description: '', status: 'IN_PROGRESS', priority: 0, assignedTo: { type: 'AGENT', id: 'agent-1', name: 'Agent' }, createdAt: '', updatedAt: '', lastEvent: null },
          jobId: 'job-9'
        }), { status: 202 })
      }
      return new Response(JSON.stringify(detail))
