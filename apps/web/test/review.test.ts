    ...
    const fetch = vi.fn(async (path: string, options: RequestInit = {}) => {
      if (path.endsWith('/approve')) {
        return new Response(JSON.stringify({
          ...
          issue: { id: 'issue-1', projectId: 'project-a', title: 'Fix', description: '', status: 'DONE', priority: 0, assignedAgentId: 'agent-1', createdAt: '', updatedAt: '' }
        }))
      }
    ...
    const fetch = vi.fn(async (path: string, options: RequestInit = {}) => {
      if (path.endsWith('/request-changes')) {
        ...
        return new Response(JSON.stringify({
          ...
          issue: { id: 'issue-1', projectId: 'project-a', title: 'Fix', description: '', status: 'IN_PROGRESS', priority: 0, assignedAgentId: 'agent-1', createdAt: '', updatedAt: '' },
          jobId: 'job-9'
        }), { status: 202 })
      }
      return new Response(JSON.stringify(detail))
