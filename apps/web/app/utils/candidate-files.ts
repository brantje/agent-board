...
function payloadStat(value: unknown) {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : 0
}

export function statsFromFileChanges(fileChanges: EventEvidence[]): Map<string, CandidateFileStats> {
  ...
  for (const event of fileChanges) {
    ...
    if (!path) continue
    const added = payloadStat(event.payload?.added)
    const removed = payloadStat(event.payload?.removed)
...
export async function loadReviewFileStats(
  ...
  const fromEvents = statsFromFileChanges(evidence.fileChanges)
