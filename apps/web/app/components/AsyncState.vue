...
defineProps<{ pending?: boolean; error?: Error; empty?: boolean; emptyTitle?: string; emptyDescription?: string }>()
defineEmits<{ retry: [] }>()
</script>
<template>
  ...
  <UEmpty v-else-if="empty" :title="emptyTitle || 'Nothing here yet'" :description="emptyDescription" />
  <slot v-else />
</template>
