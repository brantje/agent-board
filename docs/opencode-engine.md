...
3. creates one native OpenCode session in the Execution Session working directory so host runners and Docker share the same OpenCode project as `opencode serve` (a hardcoded `/workspace` path is a different OpenCode project on persistent hosts);
