...
When an authoritative Agent intentionally requests focused work through a comment, it must pass stable Agent IDs explicitly in `mentionAgentIds` and choose only from the server-provided available delegation targets; the model must never guess IDs. The backend revalidates every target at posting time. An accepted Agent-authored mention uses the same canonical parent-Run delegation command and serialized Workspace handoff as `delegate_task`; an expected policy/eligibility conflict instead persists the comment with a safe `BLOCKED` mention outcome and creates no delegated Run.
