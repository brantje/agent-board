...
Every executing OpenCode Run receives the trusted `publish_issue_comment(body, mentionAgentIds?)` capability. Project, Issue, Agent and source Run identity come from Agent Board's trusted execution context, and the durable native tool-part ID is used as the Run-scoped idempotency key. `mentionAgentIds` is optional structured intent: plain `@name` prose remains ordinary comment content and is never reparsed for routing.

When an authoritative Agent intentionally requests focused work through a comment, it may pass at most one stable Agent ID in `mentionAgentIds` and must choose it from the server-provided available delegation targets; the model must never guess IDs. Agent-authored publication supports one structured target because canonical parent execution can yield only one serialized Workspace handoff at a time. The backend revalidates the target at posting time. An accepted Agent-authored mention uses the same canonical parent-Run delegation command and serialized Workspace handoff as `delegate_task`; an expected policy/eligibility conflict instead persists the comment with a safe `BLOCKED` mention outcome and creates no delegated Run.

Publication is explicit: ordinary assistant output, reasoning, command/test output, files, logs, Run Events and status transitions are never copied into Issue comments. Completed native comment-tool parts are reconciled after attach, reconnect and normal completion. Replay uses the same tool-part request identity, returning the same canonical comment/delegation rather than multiplying either. If a recovered comment-tool completion had already accepted delegated work, reconciliation raises the same canonical delegation-handoff signal used by `delegate_task` so Workspace ownership is not stranded.

A delegated Run may publish a finding or handoff tied to its own Run. It still receives neither `set_issue_status` nor an effective nested-delegation capability; therefore structured mention intent from delegated execution is revalidated and blocked by the canonical delegation rules rather than creating a second-level child. Human blocking input continues to use OpenCode's native Question capability. Comment publication never transfers Issue ownership or mutates Board status.

Comment-triggered Runs receive only the comments durably linked to their shared work request as additional bootstrap context. Each entry preserves comment ID, author identity/type, body or tombstone state, parent/root identity and trigger/routing reason. This bounded execution-time projection is resolved before the initial prompt is admitted, so the existing immutable Execution Session admission-prompt record makes restart/reattach deterministic even if discussion state changes later. Coalescing never sends a second prompt into a live OpenCode session. Additional discussion remains available through `read_issue_discussion` rather than dumping the full Issue history into every prompt.
