...
Delegation acceptance alone is not a Workspace handoff. The ordinary child Run and START job remain held until the accepted native `delegate_task` reaches completed state with matching call ID, target Agent and task. The parent then crosses the same Workspace/Git hand-back boundary used by ordinary execution. After that hand-back is durable, the parent's fenced scheduler transition to `PAUSED` atomically yields execution ownership and releases the existing child START job. The delegate writes through the same Issue Workspace and Issue branch, and later ordinary execution consumes those changes through normal Workspace materialization/transfer. This phase does not define delegated outcomes, automatic parent continuation, cancellation propagation, or Squad-aware delegation.
...
`TestOpenCodeDockerOpenRouterDelegationEndToEnd` in `apps/server/internal/runexec/opencode_delegation_integration_test.go` is the live phase-2 delegation proof. A real OpenRouter-backed parent OpenCode Agent invokes `delegate_task` itself. The test requires one durable delegation record and ordinary target Run, canonical parent/child lineage, a bounded child task, durable completed-tool evidence, unchanged Issue ownership/status, and absence of authoritative status/delegation tools in the child. Permission acceptance alone does not release the child: the native `delegate_task` must complete, the parent Workspace must be finalized/synchronized through the normal hand-back path, and the parent must reach `PAUSED` before the existing child becomes runnable. The test does not cancel the parent merely to force child execution.
