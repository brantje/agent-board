...
- canonical Agent delegation with explicit Allow delegation, ordinary delegated Runs, durable parent/child lineage and serialized same-Workspace/same-branch handoff.
...
## Current — delegated result and parent continuation

Canonical delegation and serialized delegated Workspace handoff are implemented. The current collaboration work completes the lifecycle without creating a delegation subsystem.

Complete #140:

- bounded attributable delegated result;
- result linked to normal Run evidence rather than duplicated logs/state;
- durable exactly-once parent continuation intent;
- cancellation/failure propagation through canonical Run lifecycle;
- Issue/Review authority remains with the parent.

### Collaboration follow-through

After delegated outcomes and durable parent continuation are complete:

- Squad-aware delegation to eligible Agent members;
- human handoff/notification behavior where a concrete workflow requires it;
- broader Agent messaging/wake behavior only when needed.
