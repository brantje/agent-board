...
Structured Agent/Squad mentions are separate durable records attached to the comment. Each mention stores a typed stable target ID, deterministic position, the current resolved Agent when dispatch occurs, terminal posting outcome, safe reason code when blocked, and only the minimum canonical delegation linkage needed to correlate accepted work. Agent and Squad names are read-time display data and may change without changing target identity. Plain text that resembles `@name` never creates a structured mention or requests work.
...
When no explicit structured target exists, a human comment may resolve at most one implicit target. Routing is deterministic: a direct reply to an Agent-authored comment routes to that Agent; otherwise a reply routes only when exactly one Agent has authored a comment in that discussion thread; otherwise a top-level comment may fall back to the current `AGENT` Issue assignee or the current `SQUAD` Issue owner and its leader. Ambiguous multi-Agent threads, User/unassigned ownership, and replies with no unique Agent participant have no implicit target. Squad membership never causes automatic fanout. Agent-authored comments never gain this implicit behavior; they continue to require explicit structured target intent across the trusted Run boundary.
