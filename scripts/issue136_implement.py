...
def run(args: list[str], cwd: Path | None = None) -> str:
    ...
    result = subprocess.run(args, cwd=cwd or ROOT, check=True, text=True, capture_output=True)
    if result.stdout:
...
def run_shell(command: str, cwd: Path | None = None) -> None:
    ...
    subprocess.run(["bash", "-lc", command], cwd=cwd or ROOT, check=True)
...
def has_commit(subject: str) -> bool:
    subjects = run(["git", "log", "--format=%s", "main..HEAD"])
    return subject in subjects.splitlines()


def commit(subject: str, files: list[str]) -> str:
    run(["git", "add", *files])
...
def step1() -> None:
    subject = "feat(#136): add Squad issue ownership"
    if has_commit(subject):
        print(f"skip existing: {subject}")
        return

    replace("packages/database/schema.sql", "assignee_type IN ('USER','AGENT')", "assignee_type IN ('USER','AGENT','SQUAD')")
    replace(
        "apps/server/internal/app/assignee.go",
        '(target.Type != "USER" && target.Type != "AGENT")',
        '(target.Type != "USER" && target.Type != "AGENT" && target.Type != "SQUAD")',
    )
    replace(
    ...
    path = ROOT / "apps/server/internal/store/postgres/assignee.go"
    text = path.read_text()
    marker = "\tdefault:\n\t\treturn nil, store.ErrInvalidArgument\n\t}\n\treturn value, nil\n}"
    if text.count(marker) != 1:
        raise RuntimeError("assignee.go: default marker mismatch")
    squad_case = '''\tcase "SQUAD":
\t\tvar state string
\t\tvar inProject bool
\t\terr := q.QueryRow(ctx, `
...
\t\t}
'''
    text = text.replace(marker, squad_case + marker, 1)
    old_agent_dir = "\t\t\tSELECT 'AGENT'::text,a.id::text,a.name\n\t\t\tFROM agents AS a\n\t\t\tWHERE `+eligibleAgentAssigneePredicate+`"
    new_agent_dir = old_agent_dir + "\n\t\t\tUNION ALL\n\t\t\tSELECT 'SQUAD'::text,sq.id::text,sq.name\n\t\t\tFROM squads AS sq\n\t\t\tJOIN agents AS a ON a.id=sq.leader_agent_id\n\t\t\tWHERE sq.project_id=$1 AND a.state='ENABLED' AND (a.project_id IS NULL OR a.project_id=$1)"
    if text.count(old_agent_dir) != 1:
        raise RuntimeError("assignee.go: directory marker mismatch")
    text = text.replace(old_agent_dir, new_agent_dir, 1)
    old_lock = "LOCK TABLE users, project_user_access, project_group_access, group_members, agents IN SHARE MODE"
    if text.count(old_lock) != 1:
        raise RuntimeError("assignee.go: lock marker mismatch")
    text = text.replace(old_lock, "LOCK TABLE users, project_user_access, project_group_access, group_members, agents, squads IN SHARE MODE", 1)
    path.write_text(text)

    replace_between(
        "apps/server/internal/store/postgres/squads.go",
        "func (s *Store) DeleteSquad",
        "func lockSquadAgents",
...
''',
    ...
    (ROOT / "apps/server/internal/app/squad_assignee_test.go").write_text(r'''package app
...
''')
    ...
    (ROOT / "apps/server/internal/store/postgres/squad_assignee_integration_test.go").write_text(r'''package postgres
...
''')
    ...
    run(["gofmt", "-w",
         "apps/server/internal/app/assignee.go",
         "apps/server/internal/app/squad_assignee_test.go",
         "apps/server/internal/store/postgres/assignee.go",
         "apps/server/internal/store/postgres/core.go",
         "apps/server/internal/store/postgres/squads.go",
         "apps/server/internal/store/postgres/squad_assignee_integration_test.go"])
    run_shell("go test ./internal/app -run '^TestSetIssueAssigneeAcceptsSquadTarget$'", ROOT / "apps/server")
    run_shell("go test ./internal/store/postgres -run '^TestSquadAssignee'", ROOT / "apps/server")
    commit(subject, [
...
def step2() -> None:
    subject = "feat(#136): resolve Squad leader execution"
    if has_commit(subject):
        print(f"skip existing: {subject}")
        return

    replace(
        "apps/server/internal/store/issue_enqueue.go",
    ...
    replace_between(
        ...
        "func (s *Store) enqueueIssueMutation",
        "func nextIssueRunAttempt",
        r'''func resolveIssueExecutionAgent(ctx context.Context, q assigneeQuerier, issue store.Issue) (string, bool, error) {
...
''',
    ...
    insert_before(
        ...
        "func (s *Store) StartIssueRun",
        r'''type issueExecutionCandidate struct {
	projectID string
	issueID   string
	agentID   string
}
...
''',
    ...
    replace_between(
        "apps/server/internal/store/postgres/issue_execution.go",
        "func (s *Store) GetIssueExecutionState",
        "// Candidate identity",
        r'''func (s *Store) GetIssueExecutionState(ctx context.Context, projectID, issueID string) (store.IssueExecutionState, error) {
...
''',
    ...
    replace_between(
        "apps/server/internal/store/postgres/issue_execution.go",
        "// Candidate identity",
        "// RunnableIssueExecutionScopes",
        r'''// Candidate identity is only a hint. Re-read ownership and resolve the current
...
''',
    ...
    replace_between(
        "apps/server/internal/store/postgres/issue_execution.go",
        "// RunnableIssueExecutionScopes",
        "func (s *Store) ReconcileIssueExecution",
        r'''// RunnableIssueExecutionScopes evaluates affected Project/resolved-Agent pairs
...
''',
    ...
    path = ROOT / "apps/server/internal/store/postgres/issue_execution.go"
    text = path.read_text()
    start = text.find("func (s *Store) ReconcileIssueExecution")
    if start < 0:
        raise RuntimeError("issue_execution.go: missing reconcile")
    path.write_text(text[:start] + r'''func (s *Store) ReconcileIssueExecution(ctx context.Context, filter store.IssueExecutionFilter) ([]store.Event, error) {
...
''')
    ...
    (ROOT / "apps/server/internal/store/postgres/squad_issue_execution_integration_test.go").write_text(r'''package postgres
...
''')
    ...
    run(["gofmt", "-w",
         ...
         "apps/server/internal/store/postgres/assignment.go",
         "apps/server/internal/store/postgres/issue_execution.go",
         "apps/server/internal/store/postgres/squad_issue_execution_integration_test.go"])
    run_shell("go test ./internal/store -run '^TestShouldAutoEnqueueIssue'", ROOT / "apps/server")
    run_shell("go test ./internal/store/postgres -run '^TestSquadIssue'", ROOT / "apps/server")
    commit(subject, [
        "apps/server/internal/store/issue_enqueue.go",
        "apps/server/internal/store/postgres/core.go",
        "apps/server/internal/store/postgres/assignment.go",
...
def step3() -> None:
    subject = "feat(#136): reconcile Squad leader changes"
    if has_commit(subject):
        print(f"skip existing: {subject}")
        return
...
\treturn value, translateStoreError(err, "squad")
    ...
    (ROOT / "apps/server/internal/store/postgres/squad_leader_reconciliation_integration_test.go").write_text(r'''package postgres
...
''')
    ...
    run(["gofmt", "-w", "apps/server/internal/app/squads.go", "apps/server/internal/store/postgres/squad_leader_reconciliation_integration_test.go"])
    run_shell("go test ./internal/app -run '^Test.*Squad'", ROOT / "apps/server")
    run_shell("go test ./internal/store/postgres -run '^TestSquadLeaderChange'", ROOT / "apps/server")
    commit(subject, [
        "apps/server/internal/app/squads.go",
...
def step4() -> None:
    subject = "docs(#136): expose Squad ownership contracts"
    if has_commit(subject):
        print(f"skip existing: {subject}")
        return

    p = ROOT / "packages/api/schemas/control-plane.yaml"
    text = p.read_text()
    actual = text.count("enum: [USER, AGENT]")
    if actual != 2:
        raise RuntimeError(f"control-plane.yaml: expected 2 assignee enums, found {actual}")
    p.write_text(text.replace("enum: [USER, AGENT]", "enum: [USER, AGENT, SQUAD]"))

    replace("apps/server/internal/mcpapi/issue_schemas.go", 'Enum: []any{"USER", "AGENT"}', 'Enum: []any{"USER", "AGENT", "SQUAD"}')
    replace("apps/server/internal/mcpapi/contracts.go", 'jsonschema:"User or Agent UUID"', 'jsonschema:"User, Agent, or Squad UUID"')
    replace("apps/server/internal/mcpapi/contracts.go", 'jsonschema:"USER or AGENT target; null clears ownership"', 'jsonschema:"USER, AGENT, or SQUAD target; null clears ownership"')
    replace(
        "apps/server/internal/mcpapi/issues.go",
        "Set or clear Issue ownership using USER or AGENT plus UUID. This does not change Board status or cancel Runs.",
        "Set or clear Issue ownership using USER, AGENT, or SQUAD plus UUID. This does not change Board status or cancel Runs.",
    )
    replace("apps/server/internal/mcpapi/schema_protocol_test.go", '[]string{"USER", "AGENT"}', '[]string{"USER", "AGENT", "SQUAD"}')
    replace("apps/web/app/types/api.ts", "type: 'USER' | 'AGENT'", "type: 'USER' | 'AGENT' | 'SQUAD'")

    replace(
        "packages/api/paths/issues-runs.yaml",
        "Requires Project member authority. Starts work using only the current Agent\n      assignee, preserving Issue ownership and Board status. BACKLOG, unassigned,\n      User-assigned and invalid execution configuration return conflict. All other",
        "Requires Project member authority. Starts work using the current execution\n      Agent: the Agent owner directly, or the current leader for a Squad owner. Issue\n      ownership and Board status are preserved. BACKLOG, unassigned, User-assigned\n      and invalid execution configuration return conflict. All other",
    )

    p = ROOT / "docs/domain-model.md"
    text = p.read_text()
    if "(`USER | AGENT | null`)" not in text:
        raise RuntimeError("domain-model.md ownership marker missing")
    text = text.replace("(`USER | AGENT | null`)", "(`USER | AGENT | SQUAD | null`)", 1)
    anchor = "Persistence uses the paired `assignee_type` / `assignee_id` fields; both are null or both are set."
    if anchor not in text:
        raise RuntimeError("domain-model.md assignee anchor missing")
    text = text.replace(anchor, anchor + " For `SQUAD`, `assignee_id` is the durable Squad ID; execution resolves the Squad's current leader Agent without rewriting ownership.", 1)
    p.write_text(text)

    p = ROOT / "docs/event-protocol.md"
    text = p.read_text()
    old = '`"type": "USER" | "AGENT"`'
    if old not in text:
        raise RuntimeError("event-protocol.md assignee marker missing")
    p.write_text(text.replace(old, '`"type": "USER" | "AGENT" | "SQUAD"`', 1))

    p = ROOT / "docs/go-issue-assignment.md"
    text = p.read_text()
    old = '`type` may be `USER` or `AGENT`. Use `{ "assignedTo": null }` to clear ownership.'
    if old not in text:
        raise RuntimeError("go-issue-assignment.md ownership marker missing")
    new = '`type` may be `USER`, `AGENT`, or `SQUAD`. Use `{ "assignedTo": null }` to clear ownership. A Squad remains the persisted owner; its current leader Agent is resolved only when execution is created or reconciled.'
    p.write_text(text.replace(old, new, 1))

    p = ROOT / "docs/scheduler.md"
    text = p.read_text()
    old = "- assigning/reassigning an Agent (including initial ownership on creation) enqueues in every status except `BACKLOG`;"
    if old not in text:
        raise RuntimeError("scheduler.md assignment policy marker missing")
    new = "- assigning/reassigning an Agent or Squad (including initial ownership on creation) enqueues in every status except `BACKLOG`; a Squad resolves its current leader Agent before normal Run creation;"
    p.write_text(text.replace(old, new, 1))

    run(["gofmt", "-w", "apps/server/internal/mcpapi/contracts.go", "apps/server/internal/mcpapi/issue_schemas.go", "apps/server/internal/mcpapi/issues.go", "apps/server/internal/mcpapi/schema_protocol_test.go"])
    run_shell("go test ./internal/mcpapi ./internal/httpapi", ROOT / "apps/server")
    run(["pnpm", "install", "--frozen-lockfile"])
    run(["pnpm", "typecheck"])
    commit(subject, [
        "packages/api/schemas/control-plane.yaml",
        "packages/api/paths/issues-runs.yaml",
        "apps/server/internal/mcpapi/contracts.go",
        "apps/server/internal/mcpapi/issue_schemas.go",
        "apps/server/internal/mcpapi/issues.go",
        "apps/server/internal/mcpapi/schema_protocol_test.go",
        "apps/web/app/types/api.ts",
        "docs/domain-model.md",
        "docs/event-protocol.md",
        "docs/go-issue-assignment.md",
        "docs/scheduler.md",
    ])


def verify() -> None:
    run_shell("go test ./internal/app ./internal/store ./internal/store/postgres ./internal/httpapi ./internal/mcpapi", ROOT / "apps/server")
    run_shell("go vet ./internal/app ./internal/store/... ./internal/httpapi ./internal/mcpapi", ROOT / "apps/server")
    status = run(["git", "status", "--porcelain"])
    # Only the temporary implementation files should remain uncommitted/committed relative to product work.
    allowed = {".github/workflows/issue-136-implementation.yml", "scripts/issue136_implement.py"}
    unexpected = []
    for line in status.splitlines():
        if line and line[3:] not in allowed:
            unexpected.append(line)
    if unexpected:
        raise RuntimeError(f"unexpected working tree changes: {unexpected}")


def cleanup() -> None:
    subject = "chore(#136): remove implementation workflow"
    if has_commit(subject):
        return
    run(["git", "rm", ".github/workflows/issue-136-implementation.yml", "scripts/issue136_implement.py"])
    run(["git", "commit", "-m", subject])
    sha = run(["git", "rev-parse", "HEAD"])
    run(["git", "push", "origin", f"HEAD:{BRANCH}"])
    print(f"COMMIT {subject}: {sha}", flush=True)
